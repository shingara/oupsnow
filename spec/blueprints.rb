...
Factory.define(:admin_function, :parent => :function) do |f|
  f.name 'Admin'
...
Factory.define(:project) do |pr|
  pr.name { /\w+/.gen }
  pr.description { (0..3).of { /[:paragraph:]/.generate }.join("\n") }
end


Factory.define(:project_member) do |pm|
  pm.user { Factory(:user) }
  pm.function { Factory(:function) }
end

Factory.define(:admin_project_member, :parent => :project_member) do |pm|
  pm.user { User.first || Factory(:admin) }
  pm.function { Function.admin || Factory(:admin_function) }
end
...
Factory.define(:ticket) do |t|
  ...
  t.user_creator { self.project.project_members.first.user }
